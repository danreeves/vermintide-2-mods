...
mod:hook_origin(
	...
	function(
		...
		local override_damage_profile = override_damage_profile_name
			and DamageProfileTemplates[override_damage_profile_name]
		...
		if owner_buff_extension then
			...
			num_additional_penetrations = owner_buff_extension:apply_buffs_to_value(
				num_additional_penetrations,
				"ranged_additional_penetrations"
			)
		end
		...
		local allow_friendly_fire = forced_friendly_fire
			or (
				not friendly_fire_disabled and DamageUtils.allow_friendly_fire_ranged(difficulty_settings, owner_player)
			)
		...
		for i = 1, num_hits, 1 do
			repeat
				...
				local target_settings = (damage_profile.targets and damage_profile.targets[num_penetrations + 1])
				...
				if not is_character and not hit_units[hit_unit] then
					amount_of_mass_hit = DamageUtils._projectile_hit_object(
						current_action,
						owner_unit,
						owner_player,
						owner_buff_extension,
						target_settings,
						hit_unit,
						hit_actor,
						hit_position,
				...
				elseif not hit_units[hit_unit] and is_target and not block_processing then
					amount_of_mass_hit, num_penetrations, predicted_damage, shield_blocked =
						DamageUtils._projectile_hit_character(
							current_action,
							owner_unit,
							owner_player,
							owner_buff_extension,
...
mod:hook_origin(
	...
	function(
		...
		damage_profile_id,
		max_targets,
		current_num_penetrations,
		current_amount_of_mass_hit,
		target_number
	)
		local hit_units = HIT_UNITS
		local hit_data = HIT_DATA
			...
			if hit_zone then
				...
				if hit_zone_name ~= "afro" then
					shield_blocked = AiUtils.attack_is_shield_blocked(hit_unit, owner_unit)
						and not current_action.ignore_shield_hit
		...
		if
			hit_zone_name ~= "head"
			and AiUtils.unit_alive(hit_unit)
			and breed
			and breed.hit_zones
			and breed.hit_zones.head
		then
			local owner_buff_extension = ScriptUnit.has_extension(owner_unit, "buff_system")
		...
		elseif hit_unit_player and hit_actor == unit_actor(hit_unit, "c_afro") then
			...
			if owner_player and breed and check_buffs and not shield_blocked then
				...
				local buffs_checked = DamageUtils.buff_on_attack(
					owner_unit,
					hit_unit,
					"instant_projectile",
					is_critical_strike,
					hit_zone_name,
					target_number or num_penetrations + 1,
					send_to_server,
					buff_type,
					unmodified
				)
			...
			if breed and target_health_extension:is_alive() then
				...
				if action_mass_override and action_mass_override[breed.name] then
					...
					amount_of_mass_hit = amount_of_mass_hit
						+ (
							(
								shield_blocked
								and (
									(breed.hit_mass_counts_block and breed.hit_mass_counts_block[difficulty_rank])
									or breed.hit_mass_count_block
								)
							)
							or (breed.hit_mass_counts and breed.hit_mass_counts[difficulty_rank])
							or breed.hit_mass_count
							or 1
						)
				end
			end
			...
			predicted_damage = DamageUtils.calculate_damage(
				DamageOutput,
				hit_unit,
				owner_unit,
				hit_zone_name,
			...
			if breed and not breed.is_hero then
				...
				if is_critical_strike and critical_hit_effect then
					EffectHelper.play_skinned_surface_material_effects(
						critical_hit_effect,
						world,
						hit_unit,
						hit_position,
				...
				if Managers.state.network:game() then
					if is_critical_strike and critical_hit_effect then
						EffectHelper.remote_play_skinned_surface_material_effects(
							critical_hit_effect,
							world,
							hit_position,
							hit_rotation,
			...
			elseif hit_unit_player and breed.is_hero and current_action.player_push_velocity then
				...
				local no_ranged_knockback = hit_unit_buff_extension
					and hit_unit_buff_extension:has_buff_perk("no_ranged_knockback")
				...
				if not no_ranged_knockback then
					...
					if not status_extension:is_disabled() then
						...
						locomotion:add_external_velocity(
							current_action.player_push_velocity:unbox(),
			...
			if owner_unit_alive and hit_unit_player then
				local ranged_block = DamageUtils.check_ranged_block(
					owner_unit,
					hit_unit,
					attack_direction,
					"blocked_ranged"
			...
			if deal_damage then
				...
				weapon_system:send_rpc_attack_hit(
					damage_source_id,
					attacker_unit_id,
					hit_unit_id,
					hit_zone_id,
			...
			local target_unit_armor, _, target_unit_primary_armor, _ = ActionUtils.get_target_armor(
				hit_zone_name,
				breed,
				dummy_unit_armor
			)
...
mod:hook_origin(
	...
	function(
		...
		elseif
			is_level_unit
			and not hit_units[hit_unit]
			and (GameSettingsDevelopment.allow_ranged_attacks_to_damage_props or allow_ranged_damage)
			and has_health_extension
		then
			hit_units[hit_unit] = true
			amount_of_mass_hit = amount_of_mass_hit + 1
			local target_index = math.ceil(amount_of_mass_hit)

			DamageUtils.damage_level_unit(
				hit_unit,
				owner_unit,
				hit_zone_name,
				power_level,
		...
		elseif not is_level_unit and allow_ranged_damage and has_health_extension and not owner then
			...
			if is_critical_strike and critical_hit_effect then
				EffectHelper.play_surface_material_effects(
					critical_hit_effect,
					world,
					hit_unit,
					hit_position,
			...
			if Managers.state.network:game() then
				if is_critical_strike and critical_hit_effect then
					EffectHelper.remote_play_surface_material_effects(
						critical_hit_effect,
						world,
						hit_unit,
						hit_position,
...
mod:hook_origin(ActionBeam, "client_owner_post_update", function(self, dt, t, world, can_damage)
	...
	if
		buff_extension:has_buff_type("increased_zoom")
		and status_extension:is_zooming()
		and input_extension:get("action_three")
	then
	...
	if self.state == "shooting" then
		...
		local result = PhysicsWorld.immediate_raycast_actors(
			physics_world,
			current_position,
			direction,
			range,
			...
			if hit_unit then
				...
				if health_extension then
					...
					if self.damage_timer >= current_action.damage_interval * self.ramping_interval then
						Managers.state.entity
							:system("ai_system")
							:alert_enemies_within_range(owner_unit, POSITION_LOOKUP[owner_unit], 5)
					...
					if self.damage_timer == 0 then
						...
						if health_extension then
							...
							if hit_unit ~= self.current_target then
								...
								override_damage_profile = current_action.initial_damage_profile
									or current_action.damage_profile
									or "default"
							else
							...
							DamageUtils.process_projectile_hit(
								world,
								self.item_name,
								owner_unit,
								is_server,
							...
							if health_extension:is_alive() then
								local overcharge_amount =
									PlayerUnitStatusSettings.overcharge_values[current_action.overcharge_type]
		...
		if self.beam_effect_id then
			...
			World.set_particles_variable(
				world,
				self.beam_effect_id,
				self.beam_effect_length_id,
				Vector3(0.3, distance, 0)
			)
