...
mod:hook(GenericStatusExtension, "update", function(func, self, unit, input, dt, context, t)
  ...
  local current, max = (max_fatigue_points == 0 and 0) or (self.fatigue / (max_fatigue / max_fatigue_points)), max_fatigue_points
  ...
  local cooldown = t - self.last_fatigue_gain_time < degen_delay and math.abs(t - self.last_fatigue_gain_time - degen_delay) or 0


  widget.content.stamina_text = string.format("Stamina: %.1f", max - current)
  widget.content.other_text = string.format(
    "Regen delay: %.1fs | Regen amount: %.1f",
    cooldown,
    new_degen_amount
    )
