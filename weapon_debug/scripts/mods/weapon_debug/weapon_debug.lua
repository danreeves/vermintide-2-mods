...
function mod.on_game_state_changed(status, state)
	if status == "enter" and state == "StateIngame" then
		...
		mod._world_gui = World.create_world_gui(
			mod._world,
			Matrix4x4.identity(),
			1,
			1,
...
mod:hook_origin(
	...
	"update_sphere_sweep",
	function(self, unit, input, dt, context, t, radius, collision_filter)
		...
		PhysicsWorld.prepare_actors_for_raycast(
			physics_world,
			cached_position,
			Vector3.normalize(moved_position - cached_position),
			0,
			1,
			Vector3.length_squared(moved_position - cached_position)
		)
			...
			for i = 1, num_hits, 1 do
				...
				if not Unit.is_frozen(hit_unit) then
					...
					if not hit_self then
						...
						fassert(
							actor_index,
							'No actor index found for unit ["%s"] that was hit on actor ["%s"]',
							hit_unit,
							hit_actor
						)
...
mod:hook_safe(
	PlayerProjectileImpactUnitExtension,
	"update_raycast",
	function(self, unit, input, dt, context, t, override_collision_filter)
		if mod:get("show_attack_boxes") then
			local locomotion_extension = self.locomotion_extension
...
mod:hook_origin(ActionShieldSlam, "_hit", function(self, world, _, owner_unit, current_action)
	...
	local collision_filter = "filter_melee_sweep"
	local actors, actors_n = PhysicsWorld.immediate_overlap(
		physics_world,
		"shape",
		"sphere",
		"position",
	...
	local target_breed_unit_health_extension = Unit.alive(target_breed_unit)
		and ScriptUnit.extension(target_breed_unit, "health_system")
		...
		repeat
			...
			if not target_is_friendly_player and (breed or dummy) and not hit_units[hit_unit] then
				...
				local target_hit_position = Unit.has_node(hit_unit, "j_spine")
					and Unit.world_position(hit_unit, Unit.node(hit_unit, "j_spine"))
				...
				if self:_is_infront_player(self_pos, unit_forward, hit_position) then
					...
					if distance_to_inner_position_sq <= inner_radius_sq then
						...
						if not dummy then
							ActionSweep._play_character_impact(
								self,
								is_server,
								owner_unit,
								hit_unit,
								...
								attack_direction,
								shield_blocked,
								self.melee_boost_curve_multiplier,
								is_critical_strike
							)
						end
						...
						weapon_system:send_rpc_attack_hit(
							...
							true,
							"first_hit",
							self._num_targets_hit == 1
						)
					end
				end
			elseif
				not target_is_friendly_player
	...
	for hit_unit, _ in pairs(inner_hit_units) do
		...
		if
			(breed or dummy) and self:_is_infront_player(self_pos, unit_forward, hit_position, current_action.push_dot)
		then
			local is_server = self.is_server
...
mod:hook_origin(
	...
	function(
		...
		box_size,
		hit_units,
		overlap_update_radius
	)
		local filter_name = (attack.hit_only_players and "filter_player_hit_box_check")
			or "filter_player_and_enemy_hit_box_check"
		...
		local hit_actors, num_hit_actors = PhysicsWorld.immediate_overlap(
			physics_world,
			"position",
			oobb_pos,
			"rotation",
...
mod:hook_origin(BTStormVerminAttackAction, "anim_cb_damage", function(self, unit, blackboard)
	...
	local hit_actors, _ = PhysicsWorld.immediate_overlap(
		pw,
		"position",
		oobb_pos,
		"rotation",
	...
	for target_unit, _ in pairs(hit_units) do
		...
		if action.damage then
			...
			if DamageUtils.is_player_unit(target_unit) and blocked and action.fatigue_type == "complete" then
				SurroundingAwareSystem.add_event(
					target_unit,
					"breaking_hit",
					DialogueSettings.grabbed_broadcast_range,
					"profile_name",
					ScriptUnit.extension(target_unit, "dialogue_system").context.player_profile
				)
			end
		end
	...
	for i = 1, num_actors, 1 do
		...
		if hit_unit ~= unit and not hit_units[hit_unit] then
			...
			if target_status_extension then
				...
				if
					target_status_extension.is_dodging
					and action.dodge_mitigation_radius_squared < Vector3.length_squared(to_target)
				then
					dodge = true
				end
				...
				if not dodge then
					local attack_direction_name = action.attack_directions
						and action.attack_directions[blackboard.attack_anim]

					if target_status_extension:is_disabled() then
						damage = action.damage
					elseif
						DamageUtils.check_ranged_block(
							unit,
							hit_unit,
							Vector3.normalize(to_target),
							action.shield_blocked_fatigue_type or "shield_blocked_slam"
						)
					then
						local blocked_velocity = action.player_push_speed_blocked
							* Vector3.normalize(POSITION_LOOKUP[hit_unit] - self_pos)
...
mod:hook_origin(ActionFlamethrower, "_select_targets", function(self, world, show_outline)
	...
	local ai_units = {}
	local ai_units_n = AiUtils.broadphase_query(
		player_position + player_direction * broadphase_radius,
		broadphase_radius,
		ai_units
	)
	local physics_world = World.get_data(world, "physics_world")
	...
	if mod:get("show_attack_boxes") then
		...
		QuickDrawerStay:cone(
			player_position,
			player_position + (player_direction * SPRAY_RANGE),
			r,
			Color(255, 0, 0),
			20,
			10
	...
	if ai_units_n > 0 then
		...
		for i = 1, ai_units_n, 1 do
			...
			if not hit_units[hit_unit] then
				...
				if
					(is_enemy or not ignore_hitting_allies)
					and self:_is_infront_player(player_position, player_direction, hit_position)
					and self:_check_within_cone(start_point, player_direction, hit_unit, is_enemy)
				then
...
mod:hook_origin(ActionBulletSpray, "_select_targets", function(self, world, show_outline)
	...
	local end_point = player_position
		+ player_direction * POSITION_TWEAK
		+ player_direction * (SPRAY_RANGE - SPRAY_RADIUS)
		...
		for i = 1, num_hits, 1 do
			...
			if not hit_units[hit_unit] then
				...
				if table.contains(player_and_bot_units, hit_unit) and not ignore_hitting_allies then
					if
						self:_is_infront_player(player_position, player_direction, hit_position)
						and self:_check_within_cone(player_position, player_direction, hit_unit, true)
					then
						targets[#targets + 1] = hit_unit
						hit_units[hit_unit] = true
					end
				elseif
					(breed or dummy)
					and self:_is_infront_player(player_position, player_direction, hit_position)
					and self:_check_within_cone(player_position, player_direction, hit_unit)
				then
					targets[#targets + 1] = hit_unit
					hit_units[hit_unit] = true
